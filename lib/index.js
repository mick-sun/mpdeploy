...
const methods = {
  ...
  async upload (project, config) {
    const { ver, desc, setting, proxy, robot, npm } = config
    npm && await this.packNpmManually(project, config)
  ...
  async preview (project, config) {
    const { desc, setting, workspace, proxy, robot, page, query, scene, format, npm } = config
    npm && await this.packNpmManually(project, config)
    const spinner = ora('开始上传').start()
